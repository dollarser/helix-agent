...
### 文件传输中断恢复（HXA-182 / ADR-WORKSPACE-002）
...
这是进程中断对账，不是字节断点续传或持久后台队列；现有 picker 导入/导出、跨来源传输和旧版无日志暂存文件不在此契约内。方案见 [ADR-WORKSPACE-002](../adr/workspace/002-manual-files-and-recovery.md)。
...
''';p.write_text(s)
p=Path('docs/adr/workspace/002-manual-files-and-recovery.md');s=p.read_text().replace('## References','进程中断的显式恢复由 [ADR-WORKSPACE-002](0042-manual-transfer-recovery-journal.md) 部分扩展；不改变本决策的手动/Agent 权限分离。\n\n## References');p.write_text(s)
