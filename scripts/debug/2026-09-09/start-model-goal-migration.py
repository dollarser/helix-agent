...
p=Path('docs/adr/goal/001-lifecycle-and-completion.md')
p.write_text('''# ADR-GOAL-001: 模型判断 Goal 完成，移除强制证据绑定

Status: accepted
Date: 2026-09-09
HXA: HXA-178
Deciders: Project owner（明确要求按主流机制实现，完全抛弃旧方案）
Supersedes: ADR-GOAL-001
Superseded by: none
...
完全取代 ADR-GOAL-001；部分替代 ADR-GOAL-001 的 criterion/完成语义和 ADR-GOAL-001 的证据绑定阻塞。保留预算、显式 Continue、暂停、恢复、真实 Tool 安全验证与前台服务。
...
''')
p=Path('docs/adr/goal/001-lifecycle-and-completion.md');s=p.read_text().replace('Status: accepted','Status: superseded').replace('Superseded by: none','Superseded by: ADR-GOAL-001');s+='\n当前替代决策：[ADR-GOAL-001](0040-model-judged-goal-completion.md)。本文件保留历史，不再指导生产完成机制。\n';p.write_text(s)
for name in ['0004-goal-run-wake-budget-semantics.md','0039-background-results-and-goal-blockers.md']:
 p=Path('docs/adr',name);s=p.read_text();s+='\n完成与绑定部分由 [ADR-GOAL-001](0040-model-judged-goal-completion.md) 替代；其余执行/预算/恢复机制保留。\n';p.write_text(s)
p=Path('docs/development/status.md');s=p.read_text().replace('无活动实现；本批明确授权已完成。','HXA-178 进行中：按所有者明确要求以模型报告替换强制 Goal 证据验证，见 ADR-GOAL-001。');p.write_text(s)
p=Path('docs/development/roadmap.md');s=p.read_text();s+='\n### HXA-178 模型判断 Goal 完成\n\n状态：in progress。允许 app、core/model、core/agent、core/storage、相关测试、docs、scripts/debug。完全替代 ADR-GOAL-001，见 ADR-GOAL-001。验收：模型报告与异常边界、旧数据迁移、绑定 UI 移除、双 app/core JVM、双 flavor 构建和测试 APK、spotless/detekt/lintDebug/双 app lint、独占 API29/36 和真实模型、文档/ADR/i18n/secrets。\n';p.write_text(s)
