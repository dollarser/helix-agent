...
Goal 完成采用 [ADR-GOAL-001](../adr/goal/001-lifecycle-and-completion.md)：模型提交结构化报告，Harness 处理有效性、运行状态和结算；ADR-GOAL-001 的强制条件绑定及独立 verifier 已取代。后台任务实现的是有界工具任务列表和结果回收，不能据此宣称子 Agent 或声明式 Workflow 已实现。
...
p=Path('AGENTS.md');s=p.read_text().replace('reads proposed `docs/adr/workspace/003-git-boundaries.md`','reads accepted `docs/adr/workspace/003-git-boundaries.md`');s=s.replace('until HXA-088/ADR-WORKSPACE-003 decides persistent repository ownership.', 'outside the separately scoped persistent Git implementation authorized by accepted ADR-WORKSPACE-003.');p.write_text(s)
p=Path('docs/architecture/provider-mcp-skills-modes.md');s=p.read_text().replace('且在 ADR-AGENT-004 接受前不可用','且须通过已接受 ADR-AGENT-004 的生产启用门禁后才可用');p.write_text(s)
p=Path('docs/development/main-optimization-todo.md');s=p.read_text();pos=s.index('\n');s=s[:pos]+'''\n
> 历史执行账本：下列勾选项、待办和追加日志保留各自时点。当前开发状态以 [status](status.md) 为准；Goal 完成机制已由 [ADR-GOAL-001](../adr/goal/001-lifecycle-and-completion.md) 取代 ADR-GOAL-001，旧“绑定/复核/完成证据仍待接通”不是当前任务。后续工作树提交与合并状态不能从这里的历史“已推送”或“未提交”推断。
'''+s[pos:];p.write_text(s)
p=Path('docs/adr/workspace/002-manual-files-and-recovery.md');s=p.read_text()+'''\n## 后续扩展

[ADR-WORKSPACE-002](0041-manual-file-management-mutations.md) 基于所有者的新授权扩展首轮只读能力；本记录保留共享根目录和 Agent scope 隔离决定及 HXA-173 的历史验收范围。
''';p.write_text(s)
