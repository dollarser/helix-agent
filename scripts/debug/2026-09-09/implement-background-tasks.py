...
edit('docs/development/status.md', '本批 HXA-176 已完成。ADR-AGENT-002 取代 ADR-AGENT-002；Goal 显式继续与预算不变，无新的活动实现。',
     'HXA-177 进行中：非阻塞任务列表、持久结果回收、全局前台服务与 Goal BLOCKED/PAUSED 区分。所有者已明确授权，见 ADR-GOAL-001；验收尚未完成。')
with Path('docs/development/roadmap.md').open('a') as out:
    out.write('\n### HXA-177 后台任务与 Goal 阻塞恢复\n\n状态：in progress。允许 app、core/model、core/agent、core/storage、docs 与 scripts/debug。实现跨会话任务列表、精确暂停/取消、持久结果回收、全局服务保活；BLOCKED 不直接继续，PAUSED 显式恢复。决策见 [ADR-GOAL-001](../adr/goal/001-lifecycle-and-completion.md)。验收：core/model 与 core/agent JVM、app 双变体 JVM、spotless/detekt/lint、数据库迁移及独立模拟器任务/Goal 回归。\n')
with Path('docs/adr/goal/001-lifecycle-and-completion.md').open('a') as out:
    out.write('\n2026-09-09：[ADR-GOAL-001](0039-background-results-and-goal-blockers.md) 部分替代等待语义，新增 BLOCKED 和用户暂停；其余预算、证据与显式唤醒约束保留。\n')
