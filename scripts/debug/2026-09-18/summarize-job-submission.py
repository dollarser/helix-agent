...
for api in (29, 36):
    ...
    if re.findall(r'OK \((\d+) tests?\)', output) != ['17']:
    ...
    rows.append(dict(api=api, tests=17, artifacts=artifacts, closed=closed))
destination = root / 'build' / f'{prefix}-summary.json'
destination.write_text(json.dumps(dict(tests=34, runs=rows), indent=2) + '\n')
print('34 instrumentation cases passed; both owned emulators closed; APK hashes match')
