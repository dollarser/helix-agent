...
保留为参考而不直接覆盖 main：文件依赖旧冻结产品行为，引用已 superseded 的 ADR-0005，consumer MCP 的拒绝用例不等于正向功能验收；准备阶段修改 profile/scope 的恢复和超时 worker 生命周期也需要重新核对。后续移植须遵循当前 ADR-0012/0013、现有连接检测及独占设备约束，不能直接执行历史夹具。
