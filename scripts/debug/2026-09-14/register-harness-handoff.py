...
""")
    ...
    lines.insert(index + 1, "| HXA-192 | Harness 2.0 迁移、门禁与集成 | 进行中：[执行包及真实命令](harness-2.0-next-work.md)；CLI/storage 主机及迁移 SQL 通过，Android Room/Plan 集成与完整 check-all 待 R1～R4，不提前接受 ADR-0048 |")
    matrix.write_text("\n".join(lines) + "\n")
    ...
    note = ("HXA-192（2026-09-14）Harness 分支专项：研究材料已分类；v16 迁移与 CLI 旧契约测试已局部修复，"
            ...
            "Detekt 仍有 27 项，lint 待局部处理，ADR-0048 仍 proposed。源码修复位于 `worktree-harness-2.0`，"
