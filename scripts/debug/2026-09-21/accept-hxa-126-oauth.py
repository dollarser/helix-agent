#!/usr/bin/env python3
"""Run OAuth and Connector regression on four owned API/flavor instances.
...
spec = importlib.util.spec_from_file_location("acceptance_matrix", Path(__file__).with_name("run-acceptance-matrix.py"))
...
def main():
    ...
    classes = ["com.helix.app.mcp.oauth.McpOAuthDeviceTest",
    ...
    for flavor in args.flavor or ("consumer", "developer"):
        for api in args.api or (29, 36):
            ...
            command = [sys.executable, "scripts/debug/2026-09-18/run-owned-emulator-207.py",
                       ...
                       "--recovery-setup-class", classes[0] + "#verifierAndOneTimeConsumptionSurviveRealProcessRestart",
