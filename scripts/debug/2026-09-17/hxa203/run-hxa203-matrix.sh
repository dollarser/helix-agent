#!/bin/sh
# HXA-203 slice 4 device matrix — four quadrants (flavor x api), two runs each on
...
for quadrant in "consumer 36 5554" "consumer 29 5556" "developer 36 5558" "developer 29 5560"; do
  ...
  CLEAN_PKG_MAIN="$main_pkg" CLEAN_PKG_TEST="$test_pkg" python3 "$RUNNER" \
    ...
    --classes "$classes203" --output "app/build/hxa203-device/$variant-api$api-203" \
  ...
  CLEAN_PKG_MAIN="$main_pkg" CLEAN_PKG_TEST="$test_pkg" python3 "$RUNNER" \
    ...
    --output "app/build/hxa203-device/$variant-api$api-taskjourney" \
