#!/usr/bin/env bash
# Scripted Provider + real Goal/Dispatcher/Runtime. Run under with-host-slot.py.
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
for api in 29 36; do
  python3 scripts/debug/2026-09-09/run-owned-emulator.py \
    ...
    --classes com.helix.app.proot.DetachedGoalJourneyDeviceTest \
    --output "build/${prefix}-api${api}" --timeout 600
  port=$((port + 2))
done
python3 scripts/debug/2026-09-18/summarize-job-submission.py "$prefix" 2
