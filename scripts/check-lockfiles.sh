...
if [[ "$lock_count" != "30" ]]; then
    printf 'Expected 30 dependency lock files, found %s.\n' "$lock_count" >&2
    exit 1
fi
...
readonly projects=(
    ...
    runtime:quickjs runtime:proot-core runtime:proot-client runtime:proot-app runtime:cli-client runtime:cli-app
