...
def main():
    ...
    for phase in ['prepare', 'recover', 'recover-final']:
        ...
        cmd = base + ['shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
            'com.helix.app.provider.CliOwnerProcessKillDeviceTest', '-e', 'cli.owner.phase', phase,
            '-e', 'cli.owner.provider', args.provider,
        ...
        with log.open('w') as output:
            ...
            if phase == 'prepare':
                ...
                found = re.search(r'CLI_OWNER_RECOVERED state=(CANCELLED|INTERRUPTED) job=(job_[0-9a-f]{12})', text)
