...
class ClaudeSubscriptionModelTest {
    private val request = ModelRequest("claude-test", listOf(ModelMessage(ModelRole.USER, "hello")), maxOutputTokens = 8)
    private fun vault() = CliSubscriptionCredentialVault(object : CliSecretStore {
    ...
    @Test fun sendsOnlyClaudeCredentialAndMapsStream() {
        val vault = vault().also { it.save(CliSubscriptionProvider.CLAUDE, session("claude-fixture")); it.save(CliSubscriptionProvider.CODEX, session("codex-fixture")) }
        val client = OkHttpClient.Builder().addInterceptor { chain ->
            ...
            Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(200).message("OK")
                .body(STREAM.toResponseBody()).build()
        }.build()
    ...
    @Test fun authenticationAndQuotaFailuresAreRedactedAndNeverReplay() {
        for ((status, code) in listOf(401 to ModelErrorCode.AUTH, 403 to ModelErrorCode.AUTH, 429 to ModelErrorCode.RATE_LIMITED, 503 to ModelErrorCode.SERVER_ERROR)) {
            var calls = 0
            val client = OkHttpClient.Builder().addInterceptor { chain ->
                calls++
                Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(status).message("failure")
                    .body("private-server-detail".toResponseBody()).build()
            }.build()
            ClaudeSubscriptionModel(vault().also { it.save(CliSubscriptionProvider.CLAUDE, session("fixture")) }, { error("no replay") }, client).use {
    ...
    @Test fun missingClaudeDoesNotUseCodexAndTruncatedStreamFails() {
        ...
        val response = Response.Builder().request(okhttp3.Request.Builder().url(ClaudeSubscriptionModel.URL).build())
            .protocol(Protocol.HTTP_1_1).code(200).message("OK").body("".toResponseBody()).build()
        response.use { assertTrue(readSubscriptionEvents(it, com.helix.provider.anthropic.AnthropicStreamDecoder()).last() is ModelEvent.Error) }
    }
