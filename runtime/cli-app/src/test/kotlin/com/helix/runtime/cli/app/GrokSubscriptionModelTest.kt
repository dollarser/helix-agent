...
class GrokSubscriptionModelTest {
    ...
    private fun session(expiry: Long = Long.MAX_VALUE) = CliSubscriptionSession("grok-fixture", "refresh-fixture", null, expiry)
    ...
    @Test fun refreshesGrokBeforePostingAndDecodesTerminal() {
        ...
        val client = OkHttpClient.Builder().addInterceptor { chain ->
            ...
            Response.Builder().request(chain.request()).code(200).message("OK").protocol(Protocol.HTTP_1_1)
                .body("event: response.completed\ndata: {\"type\":\"response.completed\",\"sequence_number\":0,\"response\":{\"id\":\"r\",\"status\":\"completed\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1}}}\n\n".toResponseBody()).build()
        }.build()
        GrokSubscriptionModel(vault, { refreshed = true; vault.save(CliSubscriptionProvider.GROK, session()) }, client).use {
    ...
    @Test fun missingCredentialAndQuotaAreNotSuccess() {
        ...
        GrokSubscriptionModel(empty, { error("must not refresh") }, OkHttpClient.Builder().addInterceptor { error("must not send") }.build()).use {
            assertEquals(ModelErrorCode.AUTH, (it.run(request).events.single() as ModelEvent.Error).code)
        }
        var calls = 0
        val client = OkHttpClient.Builder().addInterceptor { chain ->
            calls++
            Response.Builder().request(chain.request()).code(429).message("quota").protocol(Protocol.HTTP_1_1).body("private".toResponseBody()).build()
        }.build()
