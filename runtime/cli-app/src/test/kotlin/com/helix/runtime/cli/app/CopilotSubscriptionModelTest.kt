...
class CopilotSubscriptionModelTest {
    ...
    private fun vault() = CliSubscriptionCredentialVault(object : CliSecretStore {
        ...
        override fun put(name: String, value: String) { data[name] = value }
    ...
    private fun session(expiry: Long = Long.MAX_VALUE) = CliSubscriptionSession("copilot-fixture", "github-fixture", null, expiry)
    ...
    @Test fun refreshBeforePostUsesOnlyCopilotAccessToken() {
        ...
        val client = OkHttpClient.Builder().addInterceptor { chain ->
            ...
            Response.Builder().request(chain.request()).code(200).message("OK").protocol(Protocol.HTTP_1_1)
                .body("data: {\"choices\":[{\"index\":0,\"delta\":{\"content\":\"HELIX_OK\"},\"finish_reason\":null}]}\n\ndata: {\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\ndata: [DONE]\n\n".toResponseBody()).build()
        }.build()
        CopilotSubscriptionModel(vault, { refreshed = true; vault.save(CliSubscriptionProvider.COPILOT, session()) }, client).use {
    ...
    @Test fun missingCredentialDoesNotBorrowAnotherPlatform() {
        ...
        CopilotSubscriptionModel(vault, { error("must not refresh") }, OkHttpClient.Builder().addInterceptor { error("must not send") }.build()).use {
    ...
    @Test fun authQuotaAndTruncatedStreamsNeverSucceedOrReplay() {
        for (status in listOf(401, 403, 429, 200)) {
            ...
            val client = OkHttpClient.Builder().addInterceptor { chain ->
                calls++
                Response.Builder().request(chain.request()).code(status).message("response").protocol(Protocol.HTTP_1_1)
                    .body("private incomplete response".toResponseBody()).build()
            }.build()
            CopilotSubscriptionModel(vault().also { it.save(CliSubscriptionProvider.COPILOT, session()) }, {}, client).use {
