...
internal class ClaudeSubscriptionModel(
    ...
    private val http = SubscriptionHttpModel(vault, refresh, CliSubscriptionProvider.CLAUDE, URL,
        AnthropicRequestEncoder { error("subscription images unsupported") }::encode,
        ::AnthropicStreamDecoder,
        mapOf("anthropic-version" to "2023-06-01", "anthropic-beta" to "oauth-2025-04-20"), client)
    fun run(request: ModelRequest) = http.run(request)
    override fun close() = http.close()
    companion object { const val URL = "https://api.anthropic.com/v1/messages?beta=true" }
}
