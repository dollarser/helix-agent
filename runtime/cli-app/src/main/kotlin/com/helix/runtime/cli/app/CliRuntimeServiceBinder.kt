...
internal class CliRuntimeServiceBinder(
    ...
    override fun onTransact(
        ...
        if (BuildConfig.DEBUG && code == CliRuntimeProtocol.TRANSACTION_DEBUG_SELF_KILL &&
            callerVerifier(getCallingUid())
        ) {
    ...
    private fun writeJob(
        ...
        when (code) {
            CliRuntimeProtocol.TRANSACTION_JOB_SUBMIT -> {
                writeSubmit(reply, data, runner, jobId)
            }
            ...
            CliRuntimeProtocol.TRANSACTION_JOB_ACK_RESULT -> {
                writeRecordOrMissing(reply, runner.acknowledgeResult(jobId, data.readString().orEmpty()))
            }

            CliRuntimeProtocol.TRANSACTION_JOB_FETCH_RESULT -> {
                writeReconcile(reply, runner, jobId, consume = false)
            }

            CliRuntimeProtocol.TRANSACTION_JOB_RECONCILE -> {
                writeReconcile(reply, runner, jobId)
            }
    ...
    private fun writeSubmit(
        ...
        when (val result = runner.submit(jobId, hash, payload)) {
            ...
            CodexPayloadSubmit.RequestMismatch -> {
                reply.writeInt(CliRuntimeProtocol.REPLY_JOB_REQUEST_MISMATCH)
            }

            CodexPayloadSubmit.Busy -> {
                reply.writeInt(CliRuntimeProtocol.REPLY_JOB_BUSY)
            }

            CodexPayloadSubmit.JournalFull -> {
                reply.writeInt(CliRuntimeProtocol.REPLY_JOB_JOURNAL_FULL)
            }
        }
    }

    private fun writeRecordOrMissing(
        reply: Parcel,
        record: CodexModelJobRecord?,
    ) {
        if (record == null) {
            reply.writeInt(CliRuntimeProtocol.REPLY_JOB_NOT_FOUND)
        } else {
    ...
    private fun writeReconcile(
        ...
        if (payload == null) {
            val record =
                if (consume && prepared.record.state.terminal && prepared.record.reconciledAtEpochMillis == null) {
