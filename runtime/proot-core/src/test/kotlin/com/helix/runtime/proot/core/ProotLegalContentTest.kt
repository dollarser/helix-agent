...
class ProotLegalContentTest {
    ...
    fun theOfflineNoticeStatesNoNetworkAndSameSignatureUpdates() {
        val notice = ProotLegalContent.offlineNotice()
        val text = notice.joinToString("\n")
        assertTrue("INTERNET" in text)
        assertTrue("同签名" in text)
        assertTrue("离线" in text)
        assertTrue("永不访问" in text)
    }

    @Test
    fun theBuildManifestHeaderCarriesLockIdentity() {
        val l = lock(component("proot", "5.1.107.92"))
        val header = ProotLegalContent.buildManifestHeader(l, "f".repeat(64)).joinToString("\n")
    ...
    fun eachComponentBlockCarriesVersionUrlSourceAndLicense() {
        ...
        assertTrue("组件 proot（5.1.107.92）" in lines)
    ...
    fun theFullPageAssemblesNoticeManifestComponentsAndLicenseTextsOnce() {
        ...
        assertTrue("PRoot Runtime — 许可证与来源" in page)
        assertTrue("离线声明" in page)
        assertTrue("组件 alpine-rootfs（3.22.5）" in page)
        ...
        assertTrue("—— 许可证全文: licenses/LGPL-3.0-or-later.txt" in page)
    }
