...
/**
 * Incremental Server-Sent Events reader for the Anthropic Messages stream
 * (HXA-024, doc 02 section 6.2: the SSE parser must handle a line split across
 * chunks, multi-line data, UTF-8 byte boundaries and the stream terminators).
 *
 * Independent implementation of the protocol island (roadmap HXA-024): the
 * Anthropic Messages API uses typed `event:` + `data:` pairs
 * (`message_start`, `content_block_start`, `content_block_delta`,
 * `content_block_stop`, `message_delta`, `message_stop`, `ping`, `error`) —
 * unlike Chat Completions' data-only frames and without a `[DONE]` token
 * (the typed `message_stop` event marks the end).
 *
 * Contract (WHATWG SSE, same discipline as the sibling adapters):
 ...
 * - lines starting with `:` are comments; `event:` sets the type for the next
 *   dispatch, `data: value` strips exactly one leading space;
 * - an empty line dispatches the buffered event: multiple `data` lines join
 *   with LF; an empty event type defaults to `message`;
 * - [finish] dispatches a pending event that still carries data when the
 *   stream ends (lenient tail), while a genuinely truncated line is discarded
 ...
 *   stop producing events (the decoder maps the failure to
 ...
 */
internal class AnthropicSseReader {
    private val utf8Tail = ArrayList<Byte>(3)
    private val line = StringBuilder()
    private val data = StringBuilder()
    private var pending = ArrayList<AnthropicSseEvent>()
    ...
    fun feed(chunk: ByteArray): List<AnthropicSseEvent> {
        if (!failed) {
            if (utf8Tail.isEmpty()) {
                decodeInto(chunk)
            } else {
    ...
    fun finish(): List<AnthropicSseEvent> {
    ...
    private fun dispatch() {
        ...
        eventType = null
        data.setLength(0)
        if (payload.isNotEmpty()) pending += AnthropicSseEvent(type, payload)
    }
...
internal data class AnthropicSseEvent(
    val type: String,
    val data: String,
)
