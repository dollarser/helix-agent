...
class GoalComposerStartDeviceTest {
    ...
    @Test fun creationKeepsFullPromptAndResumesWithoutResettingBudget() =
        withStorage { storage ->
            ...
            assertEquals(
                prompt,
                storage.messages.readContent(
                    storage.messages.listBySession("session").first {
                        it.role ==
                            "USER"
                    },
                ),
    ...
    @Test fun blockedGoalDoesNotCreateAReplacementAndOtherSessionIsIndependent() =
        withStorage { storage ->
            val helper = GoalComposerStart(storage, clock) { UUID.randomUUID().toString() }
    ...
    @Test fun failedFirstTurnRollsBackGoalCreation() =
        withStorage { storage ->
            val helper = GoalComposerStart(storage, clock) { UUID.randomUUID().toString() }
