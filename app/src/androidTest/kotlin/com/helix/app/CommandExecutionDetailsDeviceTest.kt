...
class CommandExecutionDetailsDeviceTest {
    ...
    fun chatToolRowOpensTheDetailPageAndBackReturnsToTheConversation() {
        ...
        compose.waitForIdle()
        stopAwait { chat.screen.value.openSessionId == SESSION }
        scrollToConversationNode("tool-row-$CALL_FAILED")
    ...
    fun openSessionFromTheDetailPageSwitchesToTheOwningSession() {
        ...
        scrollToConversationNode("tool-row-$CALL_SUCCEEDED")
        assertEquals("opening the owning session must start nothing", before, fixtureFacts(storage))
    }

    private fun scrollToConversationNode(tag: String) {
        compose.waitUntil(ASYNC_UI_TIMEOUT_MILLIS) {
            containerFromApp()
                .chatService.screen.value.toolTimeline
                .any { it.callId == CALL_SUCCEEDED } &&
                compose.onAllNodesWithTag("chat-timeline").fetchSemanticsNodes().isNotEmpty()
        }
        // The conversation follows its last item. Earlier tool rows may be outside
        // LazyColumn composition; waiting for their nodes without scrolling cannot work.
