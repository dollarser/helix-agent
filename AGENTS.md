...
- Every tool call goes through schema validation, capability/policy, authorization resolution, execution limits, verification and audit. Authorization resolution does not require a dialog for every call: accepted session presets and CUSTOM determine automatic resolution, confirmation or denial. Execution and audit follow ADR-PERMISSIONS-003; the preset implementation and device acceptance are recorded in the HXA-209 completion record.
