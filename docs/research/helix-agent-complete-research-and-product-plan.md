...
| Plan 与步骤 | [PlanReviewService](../../app/src/main/kotlin/com/helix/app/plan/PlanReviewService.kt)、[PlanReviewDialog](../../app/src/main/kotlin/com/helix/app/ui/PlanReviewDialog.kt)、[TaskLedgerProjection](../../app/src/main/kotlin/com/helix/app/todo/TaskLedgerProjection.kt) 已存在 | [HXA-192](../development/tasks/HXA-192.md) 仍需用户审阅→执行→当前授权的完整设备证据；元数据不是工具批准 |
...
| 用户问题 | 应有体验 | 当前任务入口 |
| --- | --- | --- |
| 不知道任务做到哪里 | 真实动作、等待原因、取消中的结算状态；不伪造百分比 | [202](../development/tasks/HXA-202.md) |
| 找不到命令输出或返回工作区 | 任务→命令详情→输出/产物→来源会话可直接导航 | [194](../development/tasks/HXA-194.md)、202 |
| 生成了文件但打不开 | 展示真实scope、来源与可用性；区分删除、变更、撤权、不支持格式 | [203](../development/tasks/HXA-203.md) |
