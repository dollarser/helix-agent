...
本次 Helix 源码基准为 main `3f5039e4`，复核生产循环、批次协调、请求装配、取消/重试、结果投影与预算路径，并对照当前状态、有效 ADR 和完成记录。HXA-196 独立分支 `codex/hxa-196-detached-jobs` 的 `63829af2` 只作为未合并切片说明，不计入 main 能力。DSH 继续采用可复查的固定快照 `0d1f50007f9bca3f52b06e1c3074fa14d5fb0720`，本次重新读取其 Agent、默认 Loop、Session 与 subagent 文档，不声称它是最新版本或所有 npm Profile 的行为。Codex/CC 为核验日的官方在线文档，后续可能变化。本轮是源码与文档核验，没有重新执行功能设备验收，也没有四产品同条件性能横评或真实账号端到端比较。
...
| 命令日志/后台执行 | 桌面工具可独立观察运行进程 | 195 实时日志已交付；196 核心在未合并分支，生产接线与真机边界待补 | 完成已有 HXA，不复制桌面常驻策略 |
...
| 顺序 | 工作 | 是否已有任务 | 交付判断 |
| --- | --- | --- | --- |
| 近期 | 196 从核心走到可授权启动、观察、取消和结果回收 | 已有，核心切片未合并 | 功能接线、取消/重启竞态及真机边界分别验收；不只验证进程能活着 |
