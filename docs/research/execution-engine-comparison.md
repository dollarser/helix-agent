...
同日后续更新：196 平台/核心切片已合入本地 main，尚未完成生产工具接线；授权与 Runtime 修复及设备证据见[收敛记录](../bug-fixes/2026-09-18-authorization-runtime-convergence.md)。下文研究基线与外部产品核验日期保留，不将后续修复视为四产品横评。

2026-09-22 专项复核：[对话历史、模型上下文与运行中用户干预](conversation-context-and-steering.md)补充最新发送/取消路径、独立 fork 分支与 Codex/Claude Code 官方交互，旧研究中的对应差距以该页限定快照为准。

## 1. 结论与阅读范围

Helix 已具备完整的单 Agent 模型—工具循环、逐调用持久结算、会话授权、Goal 连续运行、预算与上下文压缩基础。主要差距是运行中的用户干预、普通输入排队、扩展从配置到实际使用的闭环，以及可移植的调试记录。命令详情、产物交付、恢复/准备界面和一次性 Job 实时输出已分别由194/203/204/205/195交付；后台 Job 的产品接线仍未闭合。不能把这些差距归因于“Android 必须使用重状态机”，也不能用增加 Step 表、重写事件引擎或开放任意插件来代替产品闭环。

建议先推进已有后台 Job 与扩展闭环任务，保留恢复/准备/实时输出回归，再评审持久输入队列、同 Turn 转向及 JSONL 导出；有界只读委托按真实收益决定是否启用。保留 Android 生命周期、Room 事实源和统一工具授权，不以桌面功能数量作为目标。
...
研究阶段的 Helix 源码基准为 main `3f5039e4`，复核生产循环、批次协调、请求装配、取消/重试、结果投影与预算路径，并对照状态、有效 ADR 和完成记录。当时 HXA-196 分支 `63829af2` 尚未合并；同日后续以 `32de8e8d` 合入平台/核心切片，仍不代表生产工具接线完成。DSH 采用可复查的固定快照 `0d1f50007f9bca3f52b06e1c3074fa14d5fb0720`，研究时重新读取其 Agent、默认 Loop、Session 与 subagent 文档，不声称它是最新版本或所有 npm Profile 的行为。Codex/CC 为核验日的官方在线文档，后续可能变化。研究阶段没有重新执行设备验收；后续 Helix 修复验证另见收敛记录，仍没有四产品同条件性能横评或真实账号端到端比较。
...
### 2.1 执行引擎、模型 Provider 与代码 Runtime

| 层 | 回答的问题 | Helix 的实际职责 |
| --- | --- | --- |
| 模型 Provider | 向哪个模型发什么请求，如何接收流和用量？ | 适配模型协议与能力；不决定工具是否获准执行 |
| Agent 执行引擎 | 现在该请求模型、运行工具、等待用户还是结束？ | 上下文、循环、准入、记账、结算、取消与后继调度 |
| 工具管线 | 这一次操作是否允许，如何执行并验证？ | schema、Capability、Policy、授权解析、限制、验证、审计 |
| 代码/命令 Runtime | 这段代码或这个进程怎么跑、怎么停止？ | QuickJS/PRoot 的执行、输出、期限、进程与 Job 生命周期 |
| 应用服务与 UI | 用户如何配置、观察、管理与取回结果？ | 设置、准备、文件管理、任务导航、日志与产物展示 |

代码里的 `AgentRuntime` 是引擎对宿主暴露的 submit/cancel/observe 接口，不能因为名称含 Runtime，就与 `runtime/quickjs`、`runtime/proot-*` 等代码执行底座混为一谈。模型可以远程推理，而 Helix 的控制、授权和本机工具执行仍在手机。MCP 工具通常访问用户配置的外部服务，不必经过本地代码 Runtime。

“命令退出码为 0”“ToolCall 成功”“Turn 正常结束”“Goal 达成”分别是进程、工具、请求处理和目标层的结论；前一层成功不能自动证明后一层任务完成。
...
- 当前 `ChatService.retry()` 有两条路径：普通重试新建 Turn、重用原用户消息并复核附件；符合预算继续条件时，改为从已有结果新建有界 Turn。两者都不是任意模型步骤的原地恢复。
...
### 3.6 哪些内容进模型，哪些只是展示或日志

Room 与内容存储保存事实；模型请求和 UI 从这些事实产生不同投影。显示收起不等于模型看不到，落盘也不等于下一次请求全部发送。

| 信息 | 模型请求 | 用户界面与持久记录 |
| --- | --- | --- |
| 当前用户输入、必要历史和附件 | 由请求装配器按历史/附件/容量契约物化；不是整个手机文件目录 | 消息与附件绑定可追溯 |
| 系统指令、环境、模式与适用项目指令 | 请求组装时加入；任务请求记录提示来源快照 | 不作为普通聊天气泡重复展示 |
| 工具名称、描述、参数 schema | 通过模式、禁用和发现窗口等筛选后进入 tools | Registry/配置决定候选；看到 schema 不等于已获执行授权 |
| assistant 工具调用及 TOOL 结果 | 保留协议配对；用于下一轮判断成功、失败和如何继续 | 工具时间线可展开，聊天正文不重复整段协议 JSON |
| 大成功结果 | 满足条件时发送预览、稳定结果引用和分页入口 | 完整已验证结果保留；UI 和日志仍各自有上限 |
| 压缩摘要 | 代替覆盖范围内的历史参与请求，保留当前工作所需事实 | 原历史不删除，checkpoint 记录边界和来源 |
| 审批证明、内部审计、预算诊断、Runtime 原始实时日志 | 不自动整库注入；工具返回的必要拒绝/错误/输出仍可进入模型 | 供授权执行、排障和详情展示，不是额外的用户指令 |
| Provider 凭据与完整配置 | 认证层使用凭据，不作为对话正文 | 不应出现在聊天、导出或审计正文 |

当前大结果策略：成功输出超过 16 KiB，且该会话可用 `tool.result.read` 时，模型先收到 2048 个 Unicode code point 的预览及 `resultRef/nextOffset`。读取仍通过工具管线、验证同会话归属；禁用读取工具时不能生成不可读取的引用。当前分页每次仍校验完整原内容，不是文件随机读取性能保证。具体实现见 [ToolModelResult](../../app/src/main/kotlin/com/helix/app/chat/ToolModelResult.kt)、[请求装配](../../app/src/main/kotlin/com/helix/app/chat/ChatRequestAssembler.kt)和[数据边界契约](../adr/agent/006-model-data-budget-boundaries.md)。

例如 `code.javascript.run` 返回数据和 expected 两个数组，即使工具状态为 SUCCEEDED，模型仍需要读取结果才能判断数组是否符合预期。工具成功只说明调用完成，不证明两个数组一致。隐藏聊天中的外层调用 JSON，不能顺带删除模型所需的业务结果；同样不能因为一段用户正文看起来像 JSON 就隐藏它。

### 3.7 各类预算是否需要保留

预算分别控制模型容量、费用、循环和设备资源，不能用一个 token 开关替代。但用户也不应为了普通任务逐项理解所有门控。

| 约束 | 必要性 | 当前做法与改进方向 |
| --- | --- | --- |
| Provider 上下文窗口、输入及协议容量 | 硬边界；超出会请求失败或丢内容 | 请求前估算、保留输出余量，必要时压缩；未知窗口使用受控回退 |
| 单次输出上限 | 限制一次生成长度和消耗 | 与剩余额度取交集；明确区别于累计 token |
| Turn 总 token | 限制本次工作累计开销 | 已有准入与结算；主设置突出此项，已有自定义值不静默覆盖 |
| 模型调用数、工具轮数 | 防止廉价空转、短输出反复调用 | 保留高级限制，分别诊断；不能把工具轮计数称为业务进度 |
| Goal 累计 token/调用/时间 | 防止跨 Turn 继续绕过总额度 | 保留持久记账与每轮准入；压缩调用也不能免计费 |
| 工具期限、输出/磁盘/并发上限 | 防止进程失控或手机资源耗尽 | 由执行域落实；模型 token 很少也可能产生巨大文件 |

这些限制符合 Agent 工程中有界执行的需要，但不意味着主流产品采用相同字段、默认值或持久化方式。不能据界面上没有预算选项推断其完全无约束，也不能声称 Helix 多设一个限制就更可靠。后续用预算中止原因分布、任务成功率、用户手动继续次数、实际用量与估算偏差调参；不以无限自动续 Turn 绕开预算。普通预算停止已有明确继续入口，Goal 与未知副作用仍走各自恢复契约。

### 3.8 Android 上的生存边界与 HXA-196 分支状态

main 的 dataSync 前台服务覆盖活动传输阶段，等待审批和停止推进时退出；它不是永久守护进程。已有 PRoot Job 对账与日志不等于任意命令都能脱离主进程持续运行。进程死亡后，数据库可恢复的是身份和事实，不是 Kotlin 协程、模型流或 shell 的内存。
...
Helix 已有 MCP、Skill、Connector 和通用文件/代码/浏览器工具基础，普通任务不需要先创建 Goal，也不需要为每个业务写专用规划器。Plan 是可审阅工件，Todo 是进度表达，Goal 才负责跨 Turn 目标生命周期；不能把三者都当成必经流程。

主题、会话搜索、设置、能力准备、手动文件管理和用户导出属于应用服务。是否开放模型调用，应由具体用户任务决定，而不是因为已有页面或函数。扩展优先复用 MCP/Connector/Skill；Android 原生动作或现有协议无法表达时，再增加统一管线内的原生工具。不能用 developer 的 bash 替代 consumer 本应可用的能力。

当前值得测量的是工具曝光成本：MCP 有搜索/窗口机制，其他候选工具仍可能整体参与请求，并最终受到 `ModelRequest.MAX_TOOLS` 限制。按需发现可扩展，但不应先创建第二个 Registry 或授权系统。详见[工具曝光与能力复用优化建议](tool-exposure-optimization.md)。
...
| 命令日志/后台执行 | 桌面工具可独立观察运行进程 | 195 实时日志已交付；196 核心已合入，生产接线与真机边界待补 | 完成已有 HXA，不复制桌面常驻策略 |
| 可移植会话轨迹 | CC 有本地 JSONL；DSH 持久后端可保存事件日志 | Room/ContentStore 有事实，按会话 JSONL 导出仍 proposed | 用户主动导出，不增加运行时双写事实源 |
| 能力发现与扩展使用 | 各有工具/扩展接入，宿主决定生命周期 | 已有基础；207 使用闭环待做，混合工具曝光成本尚待测量 | 先复用现有协议，再按证据优化按需曝光 |
...
执行引擎方向可按以下边界拆分，避免将候选研究挤进已授权任务：

| 顺序 | 工作 | 是否已有任务 | 交付判断 |
| --- | --- | --- | --- |
| 近期 | 196 从核心走到可授权启动、观察、取消和结果回收 | 已有，核心切片已合入 | 功能接线、取消/重启竞态及真机边界分别验收；不只验证进程能活着 |
| 近期，可与 Runtime 分工 | 207 扩展添加到实际调用闭环 | 已有 | 添加、启用、发现、调用、禁用与修复全程可用；fixture 不替代外部账号验收 |
| 下一轮独立评审 | JSONL 导出、工具曝光基线 | 导出有 proposed ADR；曝光有建议文档 | 前者能关联执行事实，后者测到 schema 成本与遗漏；都不先重写引擎 |
| 下一轮交互契约 | 持久消息排队与边界 steering | 尚需决策与任务 | 输入接收、生效、取消、Goal 竞争、重启后去向明确 |
| 持续质量工作 | 压缩保真、恢复信息、预算诊断 | 已有基础，按实测缺陷切片 | 减少遗漏条件、重复工作与无法解释的停止；不重复宣称从零开发 |
| 收益证明之后 | 有界只读委托 | 有设计/Spike，未生产启用 | 同任务同预算确有收益，且端侧并发、取消、资源和恢复可控 |

191 深色主题和206整体产品验收仍按原路线推进，它们不因执行引擎研究而失去优先级；197手动PTY也不应被误设为输入队列或工具曝光优化的前置条件。
...
### 6.7 已有需求、待立项：按会话导出 JSONL

当前 Helix 没有每会话独立 JSONL 对话文件，但已有 Room 关系记录和 ContentStore 正文；这不是“没有持久历史”。JSONL 的价值在于让外部调试、检索与评测工具方便地读取稳定记录，而不是其文件格式天然比数据库更能恢复执行。

[ADR-AGENT-005](../adr/agent/005-session-jsonl-export.md)仍为 proposed：按用户操作导出消息、模型调用、工具调用及结果、压缩边界、用量和终止原因，保留稳定 ID、大内容引用及格式版本。实现应从一致快照流式导出、限制内存和暂存、明确取消与不完整输出，缺失历史事实如实标记。导出不是重新运行，也不要求每次执行同时双写 Room 和 JSONL。首版不包含自动上传、导入、跨设备恢复或自包含全量备份。

这是适合端侧的可观测性补足，应作为应用操作提供；没有具体 Agent 调试用户场景前，不必再添加“模型导出自身全部记录”的工具。验收重点是外部解析器能重建顺序与关联，而不是仅验证每行 JSON 能解析。
