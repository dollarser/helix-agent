...
4. **无限制降级安装与干净替换**：
   ...
   - 检测已安装同名目标，若内容哈希发生变更（升级、降级或版本替换），当前先卸载旧版本再写入新版本；成功路径不证明失败保留或强杀安全，改进归 HXA-129；同版本安装保持幂等。
5. **国际化资源严谨对齐**：
...
| **扩展下一项** | **HXA-129** | **Connector 安全替换、安装归属与会话启停** | 先审查 ADR-CONNECTORS-003；完整准备后提交、共享归属保护、未变凭据保留、会话独立启停及简短更新摘要，不要求代码 Diff 或多阶段 journal |
