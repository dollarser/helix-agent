...
- [ ] 继续定位浏览器 JNI 弱引用/Binder 累积问题（HXA-152 已补齐 API29/36 原生 WebView 与无 WebView 的 Autofill 对照，四组 FAIL，见 [调查记录](webview-native-reference-investigation.md)），复用五组原生对照失败证据；有针对性修复后再重跑对照，不以强制 GC 或放宽门限掩盖。
