...
历史复核基线保留12项未闭合义务：3项收尾验收、2项集成验收、3项待决策、4项发行队列。没有仅因缺完成文件而要求重做已有功能，也没有把撤销旧范围伪装为测试通过。本轮另新增4项已立规格的开发需求（214～217，214 核心已实施，215 最新消息会话内修订已独立交付，其余未启动），与历史义务分开记账；未立任务规格的 proposed ADR 不计入 HXA 任务数。

## 能力交付取舍

所有者新增授权 [HXA-213](../completion-records/HXA-213.md) 已交付：按消息创建会话分支，复用持久历史、压缩和附件，不新增 Agent 工具；独立于上述历史未闭合义务。
...
后续业务扩展优先完善MCP/Connector/Skill接入与现有通用工具；新增原生工具必须有模型主动调用的具体需求。JSONL 已获所有者实施授权并独立立项 HXA-211，首版为用户主动导出；生产子Agent、通用hooks与新编排框架不因对标而自动进入批次B/C。同Turn转向已由本轮设计请求独立规划为216，仍待对应ADR接受，不回填旧批次为已交付。此取舍不撤销已接受设计，也不关闭既有验收义务。

工具按需曝光的候选优化见[建议文档](../research/tool-exposure-optimization.md)：先测量、再评审立项，不改变当前任务依赖或验收状态。

## 新增对话交互需求（2026-09-22）

对话线剩余顺序 **214 UI收口 → 216 → 217**；215最新消息会话内修订已独立交付。整合后的产品队列在 214 后优先插入接受后的 129（安全替换与会话启停），详见[工作计划](next-work-plan.md)。214 核心已获实施授权，见[核心交接](hxa-214-core-handoff.md)；215按所有者新指令完成最新消息会话内修订（009已接受），尚未合入main；216/217未启动。

- **214 / P0**：发送回执、草稿保留、统一停止。修复现行契约，可先行；不提前改变 Goal 新输入行为。
- **215 / P1**：最新消息在原会话修订重发已独立交付，009已接受，沿214核心基线；更早历史编辑以后再做 exclusive fork。
- **216 / P1**：统一默认Queue与显式Steer，依赖214及 proposed ADR-AGENT-008接受；同步收敛Goal/Turn有效条款。技术上不依赖215，默认串行避免共享文件冲突。
- **217 / P2**：请求清单/详情/JSONL，依赖211导出、216身份契约及 proposed ADR-AGENT-010接受。
...
| C：过程可见、扩展可用 | **207 → 191（已全部交付）**（195已交付） | 长命令结束前可见真实输出；扩展完成添加到实际调用；会话搜索与深色主题可用 |
...
- **条件队列（不是自动启动）**：190真实订阅、125受保护Connector按设备/账号条件收尾；126 保留真实服务验收，130 已交付，129 待 ADR-CONNECTORS-003 接受，保留并行工作所有权。
...
| HXA-213 | 已交付 | 按消息创建会话分支 | [交付证据](../completion-records/HXA-213.md) |
| HXA-214 | 进行中 | 发送回执、草稿保留与停止一致性（核心切片，UI 待接线） | [任务规格](tasks/HXA-214.md) |
| HXA-215 | 已交付 | 最新用户消息在会话内修订重发 | [交付证据](../completion-records/HXA-215.md) |
| HXA-216 | 待决策 | 普通 Turn 与 Goal 统一排队和转向 | [任务规格](tasks/HXA-216.md) |
| HXA-217 | 待决策 | 请求上下文清单与 JSONL 可追踪性 | [任务规格](tasks/HXA-217.md) |
