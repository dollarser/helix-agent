...
本轮复核保留20项未闭合义务：3项收尾验收、8项待实现、2项集成验收、3项待决策、4项发行队列。没有仅因缺完成文件而要求重做已有功能，也没有把撤销旧范围伪装为测试通过。尚未立项的 proposed ADR 不计入 HXA 任务数。
...
后续业务扩展优先完善MCP/Connector/Skill接入与现有通用工具；新增原生工具必须有模型主动调用的具体需求。JSONL需求仍待独立设计接受与立项，首版按用户主动导出规划；生产子Agent、同Turn转向、通用hooks与新编排框架不因对标而自动进入批次B/C。此取舍不撤销已接受设计，也不关闭既有验收义务。
...
| B：能恢复、能准备 | **204 → 205**；193配套收尾 | 区分重连/对账/继续/重试，新用户完成准备，初始化失败可修复；193提供干净资产、升级及CI证据 |
| C：过程可见、扩展可用 | **195 → 207 → 191** | 长命令结束前可见真实输出；扩展完成添加到实际调用；会话搜索与深色主题可用 |
| D：核心路径验收 | **206** | 汇总A～C、已交付192/209及自身验收项，不等待完整终端 |

- **批次A（202→194→203）已全部交付**：完整用户路径证据（会话→任务→命令详情/产物→来源）见各完成记录；下一批为B（204→205，193配套收尾），按status交接。
