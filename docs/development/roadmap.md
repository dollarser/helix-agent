...
本轮复核保留25项未闭合义务：5项收尾验收、11项待实现、2项集成验收、3项待决策、4项发行队列。没有仅因缺完成文件而要求重做已有功能，也没有把撤销旧范围伪装为测试通过。
...
- **当前交接批次为A**：完成202后优先194，再203。每项单独验收和提交，不将三个HXA混为一个大切片。批次结束交付完整用户路径证据，下一批按status交接。
...
- **条件队列（不是自动启动）**：094/095 Root、190真实订阅、125受保护Connector按设备/账号条件收尾；126/129/130仍待ADR决定，保留并行工作所有权。
