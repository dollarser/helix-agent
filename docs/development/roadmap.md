...
历史复核基线保留12项未闭合义务：3项收尾验收、2项集成验收、3项待决策、4项发行队列。没有仅因缺完成文件而要求重做已有功能，也没有把撤销旧范围伪装为测试通过。本轮另新增4项已立规格的开发需求（214～217，尚未实施），与历史义务分开记账；未立任务规格的 proposed ADR 不计入 HXA 任务数。
...
对话线顺序 **214 → 215 → 216 → 217**；整合后的产品队列在 214 后优先插入接受后的 129（安全替换与会话启停），详见[工作计划](next-work-plan.md)。本轮只交付设计与规格，不启动功能实现、不自动接受 proposed。
...
| HXA-214 | 待开发 | 发送回执、草稿保留与停止一致性 | [任务规格](tasks/HXA-214.md) |
