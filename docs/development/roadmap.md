...
状态：in progress。取得独立测试账号及可公开或脱敏的真实导出样本，验证匿名/bearer 握手、工具调用、拒绝、撤销与重启。分别记录四个平台已验证子集和未知项，不用合成 fixture 代替服务验收。依赖 HXA-124；无账号时记录外部依赖。允许 app 测试、导入适配器与相关 docs/scripts。
