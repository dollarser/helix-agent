...
历史复核基线保留12项未闭合义务：3项收尾验收、2项集成验收、3项待决策、4项发行队列。没有仅因缺完成文件而要求重做已有功能，也没有把撤销旧范围伪装为测试通过。本轮另新增4项已立规格的开发需求（214～217，214 核心已授权实施，其余未启动），与历史义务分开记账；未立任务规格的 proposed ADR 不计入 HXA 任务数。
...
对话线顺序 **214 → 215 → 216 → 217**；整合后的产品队列在 214 后优先插入接受后的 129（安全替换与会话启停），详见[工作计划](next-work-plan.md)。214 核心已获实施授权，见[核心交接](hxa-214-core-handoff.md)；其他功能未启动，不自动接受 proposed。

- **214 / P0**：发送回执、草稿保留、统一停止。修复现行契约，可先行；不提前改变 Goal 新输入行为。
- **215 / P1**：编辑并分支重发，依赖213/214进入基线和 proposed ADR-AGENT-009接受。
...
| HXA-215 | 待决策 | 编辑用户消息并分支重发 | [任务规格](tasks/HXA-215.md) |
