...
实现和审查最好使用两个独立上下文。无论是否创建持久 Goal，都应按依赖持续完成用户授权的范围；一个测试、checkpoint 或 HXA 通过后继续后续工作，不把局部完成当作整个任务的停止条件。审查模型先看 diff、需求和测试结果，再看实现模型的说明。
...
只有用户明确要求建立持久 Goal 时才创建 Goal；Goal 覆盖用户授权的里程碑或范围。持续推进不以创建 Goal 为前提：checkpoint 完成后写完成记录、更新唯一状态源并通过验收矩阵，再根据路线依赖继续下一项。某项阻塞时记录具体原因，继续其他独立且已授权的工作；只有授权范围完成、用户要求停止，或缺少必要权限、信息、外部条件而无法继续有效推进时才结束，并明确列出未完成项。
