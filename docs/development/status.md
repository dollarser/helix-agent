...
| 维度 | 当前事实 |
| --- | --- |
| 已验证范围 | M0～M4：HXA-001～003、010～016、020～028、030～047；M5：HXA-050（Zipline/QuickJS spike）、HXA-051（生产执行协议 isolated Service/Binder）、HXA-052（IIFE wrapper/JSON ABI/限制，API 29/36 arm64-v8a；HXA-048 收敛项在本分支未执行） |
...
| 尚无业务执行器 | Browser、MCP、Skill、QuickJS（执行通道 + wrapper 已落地但未接 Tool，接线归 HXA-053）、PRoot/CLI、Accessibility、Root |
| 当前检查点 | M5 / HXA-052 IIFE wrapper 与输入注入完成（JSON 输入输出、host 编码参数 + 局部 `const input`、19 例 wrapper 逃逸攻击套件，55 例 androidTest × 两台设备 + 62 例 JVM 全过）；下一项为 M5 / HXA-053 QuickJS 执行接 Tool/Policy/审计 |
...
- M5 / HXA-053 QuickJS 执行接 Tool/Policy/审计：`JsExecutionClient(context).execute(JsExecuteParams, JsCancellation?): JsExecutionResult` 接 Tool 管线（同步有界、不重试、并发由调用方负责；11 状态闭合集、PROTOCOL_VERSION=2、limits 默认/上限与输入"恰一个合法 JSON 文档"、SUCCESS 输出"≤ maxOutputBytes 的单个 JSON 文档"契约见 [HXA-052 完成记录](../completion-records/HXA-052.md)）。
...
- QuickJS 执行通道已落地：主 App 内 isolated 非导出 Service（每执行唯一 `js_` + 32-hex 实例）+ 主进程 `JsExecutionClient`（同步有界、不重试；wrapper JSON ABI：输入 = 恰一个合法 JSON 文档、输出 = ≤ maxOutputBytes 的单个 JSON 文档、11 状态闭合集、PROTOCOL_VERSION=2；API 见 [HXA-052 完成记录](../completion-records/HXA-052.md)），尚未接 Tool（HXA-053 接线）；PRoot 规划使用同签名、独立 applicationId/UID 的 Runtime APK，并通过 signature-protected Binder/PFD IPC 连接。
