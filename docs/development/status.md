...
196 已完成后台启动、原绑定查询/取消、原结果收取与 Workspace 导入、Goal prepare/reject/settle 宿主组合，并在 developer 生产 Registry 注册 start/status/cancel/collect；consumer 不注册。最新完整主机门禁与双 API Job **64/64** 通过，含真实注册 executor → Runtime → ChatService 普通 Turn 预算 → 原目标导入旅程；这不是完整模型 Dispatcher/Goal 产品验收。历史时间账本、同步回归和故障恢复证据见 [196 任务记录](tasks/HXA-196.md)。闭合 Goal/Plan/Todo 元数据准入及 pending 租期禁止提前完成已接通，最新完整主机门禁与 Goal 四象限 **248/248** 通过。剩余重点为 Tasks 投影、完整 Dispatcher/Goal/恢复旅程与真机 G4。
