...
- 设备矩阵：consumer 变体在 API 36 arm64-v8a 模拟器已全绿（HXA-048 实测 47/47）；API 29 已实测但仍有两项已知失败，均非本设备列引入——`ProductionMigrationDeviceTest` ×2（`MIGRATION_1_2` 使用 `RENAME COLUMN`，需 SQLite ≥3.25/API 30+，Android 10 上 v1→v2 迁移会崩，属既有产品缺陷）与 `FilesScreenTest` ×6（API 29 模拟器慢 + 双模拟器并发跑全量套件的 `waitTag` 超时抖动，同样 6 例在 API 36 全部通过）。HXA-048 曾一并失败的 `GoalReminderTest` ×2（POST_NOTIFICATIONS 授予 helper 未对 SDK<33 设防）已修复（helper 在 API<33 时 no-op），不再是已知失败。多 ABI 与真机矩阵仍待执行。
