...
196 最新恢复切片：运行中 Runtime 实际死亡后，原 Job 的 ORPHANED 不再作为普通操作失败显示；原 Turn 已结束时，收取一次性结算未知 lease 并暂停原 Goal，重复收取不重复计费。停止尚未证实，执行占用和待处理项继续保留，不导入、不重放。完整主机门禁及 API29/36 developer 正常/死亡旅程 **6/6** 通过。仍需可靠停止证明及恢复出口、运行中主进程死亡、真机 G4；196 未关闭，详见 [196 任务记录](tasks/HXA-196.md)。
...
2026-09-20 接续：并行三工具实现已按源码复核，因效果分类、错误/取消回执及缺少持久占用/Goal 结算问题未直接采纳，main 未提交工作保留，见[交接复核](../evidence/development/hxa-196-handoff-review-2026-09-20.md)。现有四工具新增“终态未导入时主进程死亡”的真实新 PID 恢复，完整主机门禁与双 API **2/2** 通过；运行中主进程/Runtime 死亡和 G4 仍待验，不采用交接中的“只剩真机”结论。
