...
当前开发 checkpoint 为 **197 单个手动 PTY**。有界缓冲与私有 Runtime 原生 I/O 已实施，实际 PRoot/失败退出/资源边界验证见[197 任务](tasks/HXA-197.md)；会话 owner、租期与终端 UI 仍待接线。191 已完成并通过最终主题/搜索四象限。196 的本地实现与模拟器验收阶段已收敛，保留真机 HOME/锁屏/Doze 外部待验，**不记整体完成**。进入 191 前固定制品通过 Job 综合回归 **70/70**、既有 Runtime 回归 **74/74** 及 release 边界检查；最新完整主机门禁和恢复专项见 [196 任务记录](tasks/HXA-196.md)。缺真机不阻塞独立终端工作；同一时间只推进一个开发 checkpoint。
