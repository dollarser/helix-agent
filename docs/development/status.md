...
196 重启恢复出口：提交前保存系统启动次数；ORPHANED 只有在设备真实重启后才允许收取结算并释放原占用，执行状态仍保持中断，不导入未知产物或重跑命令。旧绑定首次收取建立观察，之后重启可恢复；没有系统证据或原 Runtime 记录时仍保留占用。该切片主机全门禁、API29/36 实际重启 **2/2**、正常及同次开机死亡回归 **6/6** 通过。顺带修正 Spotless Kotlin 扫描 build 产物与并行编译的竞态。普通主进程死亡的后续验证见下；仍需原记录缺失边界及真机 G4，见 [196 任务记录](tasks/HXA-196.md)。
