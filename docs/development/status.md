...
当前开发 checkpoint 为 **197 单个手动 PTY**。有界缓冲、原生 I/O、单写连接和会话记录已连接到 Runtime 工作线程，双 API 公共 Runtime 回归各 44/44；正式服务与页面尚未接线，具体范围见[197 任务](tasks/HXA-197.md)；会话 owner、租期与终端 UI 仍待接线。191 已完成并通过最终主题/搜索四象限。196 的本地实现与模拟器验收阶段已收敛，保留真机 HOME/锁屏/Doze 外部待验，**不记整体完成**。进入 191 前固定制品通过 Job 综合回归 **70/70**、既有 Runtime 回归 **74/74** 及 release 边界检查；最新完整主机门禁和恢复专项见 [196 任务记录](tasks/HXA-196.md)。缺真机不阻塞独立终端工作；同一时间只推进一个开发 checkpoint。
...
191 最终主题矩阵通过：developer 双 API 深浅模式 **32/32**；consumer **28/28** 且最终主/测试 APK 哈希与验收时一致。已覆盖真实对话框、六个订阅页面配色与实际系统栏标志，修复大字体返回箭头裁切；完整主机门禁通过，见 [191 记录](../completion-records/HXA-191.md)。API29 使用自有模拟器 root shell 设置被锁定的系统夜间模式，没有赋予应用权限。最终搜索 UI/真实进程恢复 32/32、Room 16/16 通过，191 已关闭。下一步验证 197 的组件许可证、ABI/16 KiB、PTY 与目录映射，然后接入单手动终端。主题与搜索是应用能力，不新增 Agent 工具。
