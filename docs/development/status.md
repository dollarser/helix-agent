...
| 维度 | 当前事实 |
| --- | --- |
| 已验证范围 | M0～M4：HXA-001～003、010～016、020～028、030～049；**M5 完成**：HXA-050（Zipline/QuickJS spike）、HXA-051（生产执行协议 isolated Service/Binder）、HXA-052（IIFE wrapper/JSON ABI/限制，API 29/36 arm64-v8a）、HXA-053（QuickJS 执行接 Tool/Policy/审计：`code.javascript.run` 全链路 + 11 状态回填 + §4.8 脱敏审计 + 完整代码审批，API 29/36 arm64-v8a）、HXA-054（攻击与端到端测试：8 项攻击点全链路 E2E + 设备证据，75 例 androidTest/设备 × 两台 + 84 例 JVM）；**M5A 进行中**：HXA-055（图片输入与 Provider 视觉能力：production 图片源（会话绑定 + hash/magic 重验、fail-closed）+ 端上归一化（字节门/bounds 探针/采样/EXIF 手动旋转/重编码剥离）+ `VisionLimits` 集中预算（API 29/36 测量 + Provider 上限取更严者）+ 连接测试第 5 阶段视觉探测（Completed/Refusal 证明 vision）/用户可见手动声明 + 绑定归一化 artifact 的发送/重试/恢复重验 + 出网披露图片行，API 29/36 arm64-v8a） |
...
| 当前检查点 | **M5A 进行中**：HXA-055 完成（完成记录在案；JVM 753 例 + consumer 设备套件 64 例 × 两台（API 29/36）+ spotless/detekt 全过；分支 `hxa/m5a-image-input`）；下一项为 HXA-056 文本/图片附件端到端硬化与发布边界 |
...
- M5A / HXA-056 文本/图片附件端到端硬化与发布边界：固定 fixture 完成纯文本与图片附件消息的发送/流式回复/Tool Loop/历史恢复/脱敏回归；`ACTION_SEND`/`ACTION_SEND_MULTIPLE` 文字/图片草稿（先本地导入/预览，绝不自动发送）；picker/share grant 失效、Artifact 缺失/篡改、重复发送、Provider/origin 切换、能力快照变化、请求过大、图片解压炸弹、取消与进程回收；UTF-16/PDF/PPT/DOC/音频/视频/未知二进制只验证统一稳定拒绝（无解析器/OCR/媒体解码断言）；日志/Room/audit/用户可见错误不泄露原始 URI/绝对路径/正文/base64；三 adapter golden request + ≥1 真实 vision endpoint smoke（明确记录环境，不替代 fixture）；更新用户说明（详见 [roadmap §9A](roadmap.md)）。
