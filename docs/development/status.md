...
2026-09-18 审查调整：**先修已有授权与稳定性缺陷，再继续新增功能**。优先处理新会话默认权限影响旧会话、权限保存/审计原子性及并发编辑、SAF 来源移除与浏览器标签超限错误边界；随后完成订阅 Runtime 内存/单批传输与失败结算修复，再承接已授权 196 及独立 207/191。具体证据、报告纠偏与验收出口见[审查复核](../evidence/development/review-followup-2026-09-18.md)。本轮仅审查与安排，尚未修复；不恢复 ADR 已撤销的订阅累计配额，也不以全面重构作为开发前置。
...
- **下一候选任务：HXA-207 扩展添加到使用闭环**，随后191深色主题剩余部分与206核心集成验收；本次只整合，不自动开始开发。193、195、204、205均已交付，各任务证据与范围见完成记录。
