...
更新：2026-09-20。此页只维护当前结论、下一步和未闭合边界。具体测试数字与过程见证据，不从历史 commit/设备推断当前现场。
...
当前开发 checkpoint 为 **197 单个手动 PTY**。应用侧手动服务、共享执行占用、私有前台 Service/Binder、实际 Workspace 映射及独立租期已接线；完整主机门禁与双 API Runtime 回归各 46/46 通过，具体范围见[产品会话证据](../evidence/development/hxa-197-product-session-2026-09-20.md)。终端组件与 Workspace 页面现已接线，中文 IME/真实 REPL/Activity 重建/停止结算已验证；最新完整主机门禁及双 API 各 47/47、SDK/Compose 升级后的主题 60 项见[页面证据](../evidence/development/hxa-197-terminal-page-2026-09-20.md)。下一步是产品进程死亡对账、页面重连及输出/粘贴/运行中租期专项，不关闭 197。191 已完成并通过最终主题/搜索四象限。196 的本地实现与模拟器验收阶段已收敛，保留真机 HOME/锁屏/Doze 外部待验，**不记整体完成**。进入 191 前固定制品通过 Job 综合回归 **70/70**、既有 Runtime 回归 **74/74** 及 release 边界检查；最新完整主机门禁和恢复专项见 [196 任务记录](tasks/HXA-196.md)。缺真机不阻塞独立终端工作；同一时间只推进一个开发 checkpoint。

2026-09-19 所有者已建立持续开发 Goal，执行顺序为 **196 后台 Job → 191 主题收尾 → 197 单手动 PTY → [211 会话 JSONL 导出](tasks/HXA-211.md) → 198 → 199 → 206 产品集成验收**。当前 checkpoint 为 197；每次仅一个 HXA 进行中，进入下一项前解决强制门禁的已知失败。196 已实现跨调用执行所有权、预算与结果回收，真机仍待验；197 先验证组件/许可证/目录映射再接 UI；211 按接受的 ADR-AGENT-005 实施。206 原先仅维护计划的限制由本次明确执行授权替代。外部设备或账号缺口独立记录，不计为通过；不阻断无该依赖的独立实现。191/207 的具名交付已整合到当前分支。
...
191 最终主题矩阵通过：developer 双 API 深浅模式 **32/32**；consumer **28/28** 且最终主/测试 APK 哈希与验收时一致。已覆盖真实对话框、六个订阅页面配色与实际系统栏标志，修复大字体返回箭头裁切；完整主机门禁通过，见 [191 记录](../completion-records/HXA-191.md)。API29 使用自有模拟器 root shell 设置被锁定的系统夜间模式，没有赋予应用权限。最终搜索 UI/真实进程恢复 32/32、Room 16/16 通过，191 已关闭。197 的组件、产品会话与终端页面已接线并有设备证据，下一步补实际进程死亡对账、页面退出重连及输出/输入压力和运行中租期验收。主题与搜索是应用能力，不新增 Agent 工具。
...
- **当前独立开发：197 单个手动 PTY**；191 已交付，196 保留真机待验，之后按持续 Goal 顺序完成 211、198、199 与 206。193、195、204、205均已交付，各任务证据与范围见完成记录。
