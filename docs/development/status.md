...
196 已完成后台启动、原绑定查询/取消、原结果收取与 Workspace 导入、Goal prepare/reject/settle 宿主组合，并在 developer 生产 Registry 注册 start/status/cancel/collect；consumer 不注册。后台命令已接入既有详情入口，区分提交回执、执行终态和宿主待结算，浏览只读本地凭据。最新完整主机门禁、双 API Job **64/64** 与命令详情四象限 **26/26** 通过；含真实注册 executor → Runtime → ChatService 普通 Turn 预算 → 原目标导入旅程，但不是完整模型 Dispatcher/Goal 产品验收。具体证据见 [196 任务记录](tasks/HXA-196.md)。闭合 Goal/Plan/Todo 元数据准入及 pending 租期禁止提前完成已接通，其前轮 Goal 四象限 **248/248** 证据保留。剩余重点为 Tasks 独立 Job 行与操作入口、完整 Dispatcher/Goal/恢复旅程及真机 G4。
