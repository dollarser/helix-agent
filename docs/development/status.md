...
| 当前检查点 | **M5 完成**（HXA-050～054 全部有完成记录，HXA-048/049 收敛已在 main 落地）；M5A / HXA-055 图片输入与 Provider 视觉能力由他人并行处理；**M6 进行中：HXA-060 最小 WebView 浏览器 + HXA-061 Browser snapshot + HXA-062 Browser actions + HXA-063 Browser download + HXA-064 分享/Intent/剪贴板 完成**（见下方 Completed 表 M6 行），M6 下一项为 HXA-065 通知和日历 |
...
- M6 / HXA-065 通知和日历：通知 app allowlist/time window；Calendar 先草稿再 commit。权限关闭返回 `PermissionMissing`（详见 [roadmap](roadmap.md)）。
