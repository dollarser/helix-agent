...
| 当前检查点 | **M5 完成**（HXA-050～054 全部有完成记录，HXA-048/049 收敛已在 main 落地）；M5A / HXA-055 图片输入与 Provider 视觉能力由他人并行处理；**M6 进行中：HXA-060 最小 WebView 浏览器 + HXA-061 Browser snapshot + HXA-062 Browser actions 完成**（见下方 Completed 表 M6 行），M6 下一项为 HXA-063 Browser download |
...
- M6 / HXA-063 Browser download：实现 `browser.download`（URL/重定向/MIME/size/name/target policy，下载到 Workspace/SAF）；不自动打开或执行 APK/DEX/JAR/SO。
