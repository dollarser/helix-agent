...
| E2C | 第三方订阅协议适配器 | developer/Advanced 的订阅 Provider | 独立网络 Runtime APK；当前尚未注册 |
...
- adapter 只可在 developer/Advanced 适配为 `ModelProvider`，且必须具备受支持执行底座、工具控制、
  Dispatcher/Approval/Verification/Audit 和 jobId 对账证据。consumer/store 另需供应商可核验授权。
...
E2C 当前完成登录、固定 smoke 与 Runtime 私有 journal；第三方 adapter 凭据不离开独立网络 UID。
官方 CLI 打包路线停止；developer/Advanced Provider 允许继续实现，但在跨 APK Job、统一事件和
Dispatcher/Audit 验收前不得把现有 smoke 描述为 Helix Agent 后端。consumer/store 保持关闭。
