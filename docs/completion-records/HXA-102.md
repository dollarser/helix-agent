...
- 日期：2026-09-05
- 目标：收口模型流、审批与各副作用执行器的 kill/restart 不重放语义，并按 ADR-GOAL-001 为 Goal 增加原子 durable budget accounting 与明确最大未记账窗口。
...
- 未实现/明确排除：没有将 `INTERRUPTED` 或 `NEEDS_REVIEW` 猜成成功/失败，没有后台自动继续 Goal，没有改变 ADR-GOAL-001 的用户显式 continue 语义。5 秒内尚未 checkpoint 的纯运行时长在突然断电时仍可能丢失，这是明确上界，不伪称零窗口。
...
- 决策记录：不适用；严格实现 accepted ADR-GOAL-001 的 pause/wake/budget 语义，没有修改或替代该决定。
...
详见 [M10 收尾跟进](../evidence/development/m10-closure-followup.md)。既有通过日志保留，不反向改写成端到端证据。

## 生产接线前置进展

2026-09-06：预算耗尽原子关闭 run 已修复；Room v8 增加 Goal run/Turn 关联，并在 TurnCoordinator 创建事务中写入。API 34 应用组件 15/15、存储迁移 20/20 通过，详见 [收尾跟进](../evidence/development/m10-closure-followup.md)。上层 ChatService Goal 创建/继续与模型/工具预算边界仍未接通，原集成复核限制保持有效。
...
[结果恢复缺口](../evidence/development/cli-result-durable-recovery-gap.md)仍open：接下来做主进程强杀窗口、持久结果读取/恢复界面，不能用上述正常流程测试替代。未将Goal标记完成，未启动长稳或真实账号调用。
...
源码复核确认ProotJobClient仅submit/query/cancel/reconcile；ProductionLinuxExecutor收到的output PFD落在随机临时目录，finally删除且没有持久结果定位绑定。Runtime虽保留output.zip，但未提供非破坏性重新取回；现有reconcile会记录确认并删除载荷。因此查询/停止或进程死亡后的临时残留不能充当成功结果恢复验收。后续按 [具体缺口](../evidence/development/proot-result-durable-recovery-gap.md) 补取回、私有持久化、精确确认与只读展示，工作区新写入仍走既有工具审批。
...
### 2026-09-08 ADR-GOAL-001 完成证据进程死亡边界
