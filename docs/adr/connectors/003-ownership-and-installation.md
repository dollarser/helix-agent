# ADR-CONNECTORS-003: Connector 安全替换、安装归属与会话启停

Status: accepted
Date: 2026-09-22
HXA: HXA-129
Deciders: Project owner（2026-09-22 授权收口并进入 HXA-129 实现）

## Context

Connector 安装同时涉及 Skill 快照、注册记录、端点配置和凭据。当前 MarketplaceService 在不同 hash 的替换中先卸载再安装；ConnectorService 先注册 Skill，再原子发布 Connector JSON。单文件 ATOMIC_MOVE 不能把这些操作变成同一个事务：后续失败或进程死亡可能丢失旧配置，或留下已注册但未归属的 Skill。

现有 remove 会保护其他 Connector 引用的 Skill，但不能完整表达独立用户安装的归属。问题需要解决，但不需要因此建设通用安装事务引擎、代码 Diff 页面或新的会话权限系统。本记录已获授权；设计接受不是实现或验收证据。

## Decision

采用“完整准备、一次发布、可重试清理”实现端侧安全替换。优先复用现有存储和安装入口，不新增场景执行引擎。

### 1. 稳定身份与最小归属记录

区分稳定 Connector 身份、不可变内容 hash 和显示名称。市场条目身份包含可信来源及 packageId；本地导入保留独立安装身份。不得仅凭同名或相同内容认领另一个安装。相同目标身份、相同 hash 的重复安装返回已有结果；替换请求绑定预期旧版本，旧版本变化时重新计算目标，不能覆盖并发用户修改。

清单引用准确 SkillKey（source/name/snapshotHash），最小引用记录区分独立用户安装与 Connector 引用。移除包只撤销它自己的引用和启用意图；不得禁用或删除仍属于独立用户或其他包的内容。无法证明归属的旧资产保守保留并提示修复，不按名称批量清除。

端点不因 URL 相同而跨包共享账号。安装归属不是 Tool Approval，实际调用继续通过现有 Dispatcher、Capability、Policy 和授权解析。

### 2. 每个会话独立启停 Connector

设备安装回答“有哪些包”，会话选择回答“本会话使用哪些包”。每个会话持久保存按稳定 connectorId 标识的启用集合；用户可在会话能力面板独立开关，不影响其他会话。复用已有会话设置和组件启用基础，不另建通用 scope 框架。

新会话创建时复制用户默认集合，随后独立保存；更改默认集合不追溯修改已有会话。没有默认配置时不因安装自动加入会话。已有会话迁移按原有效配置初始化，保持原有可用性；删除会话只清除它的选择，不卸载包或删除凭据。若会话创建路径支持复制/fork，则复制选择快照，不共享可变设置；目标包不存在时显示不可用，不自动安装。

会话开关控制该 Connector 提供的 Skill 和 MCP 工具。可用性要求包已提交且组件就绪、当前会话选中该来源，并满足现有组件和工具启用规则；工具显式禁用及现有授权规则仍生效。打开 Connector 不强行启用被单独禁用的工具，不自动登录、提升授权预设或修改其他来源。面板区分“已选择”和“需配置/不可用”，并提供现有修复入口。

关闭 Connector 撤销的是本会话的这一来源。相同准确 SkillKey 若仍由独立用户安装或其他已选 Connector 有效提供，可以继续可用，界面说明剩余来源；单独全局启用一个包内 Skill 不得绕过该包的会话开关，必须有独立安装归属。不同端点身份不按 URL 自动合并。用户要完全禁用某个工具时使用既有工具禁用项，不创造新的权限优先级。

选择变更在下一次上下文构建生效：不再注入关闭来源的 Skill 指令、目录和工具 schema；Skill list/read 与实际工具调用均按可信 sessionId 校验，不能只在 UI 隐藏。已经进入历史消息、摘要或正在发送的请求中的内容不会因此被擦除，也不能承诺模型忘记它；保留历史及审计，不靠删除历史实现启停。

运行中切换不要求取消整个 Turn，普通 Turn 与 Goal Turn 适用同一规则。尚未发送或正在等待批准的调用在执行前重新检查会话选择，失效则按现有失败/取消结算路径返回明确结果，不能因旧 schema 或旧审批继续执行；连接前校验一次，并在tools/call前最后一次同步校验；该校验通过是本地发送准入的线性化点。用户关闭若先完成则取消调用；若发送准入先完成则按已获准发送继续结算，不承诺关闭返回后网络绝不会再发出字节。已经发送的调用继续结算，不盲目重发，不承诺撤回远端效果。恢复后的会话重新读取持久选择，模型及扩展内容不能修改这些开关。

会话绑定稳定安装身份，普通更新保留选择并解析到当前已提交版本；目标或认证变化仍按下一节处理，不能借保持选择跳过配置。卸载后相关选择标为不可用或移除；新安装身份不能仅凭同名继承选择。此任务不提供每会话固定不同版本或独立账号副本。

### 3. 更新体验与凭据绑定

更新页只需作者更新说明、版本及简短组件变化摘要；没有说明时如实显示。无需逐行 JSON/Markdown Diff，不新增每次更新都要通过的独立审查弹窗。用户点击更新即表达对所展示目标版本的安装意图；安装不自动批准后续工具执行。

摘要绑定旧、新内容 hash。新增或改变的网络目标、认证要求在现有配置/启用入口明确展示；需要重新配置或登录时直接引导修复。目标及认证绑定完全未变时保留现有配置和凭据；URL 路径、origin、OAuth issuer/resource/client 等实际绑定变化时不得盲目复制 token，受影响端点保持未激活直至完成相应配置。其他未变端点不要求重复登录。

复用既有 Skill 与端点启用契约，包括市场独立 Skill 的显式安装流程，不新增全包统一禁用或自动授权规则。签名仅证明来源，不能替代用户更新操作、启用或工具授权。

### 4. 最小提交与恢复协议

先在暂存区完整验证候选包及所有组件；提交前不卸载旧版本、不删除旧凭据，也不让候选组件通过独立 Registry 扫描提前生效。准备失败、取消、低空间或写入错误均保留旧的有效安装。

优先使用不可变候选内容及单一权威已提交清单/指针发布目标版本，再幂等清理无引用旧内容。所有相关列表、启用和执行入口必须依据已提交版本，不能只保证 Connector 页面看起来完整。文件方案须说明同卷原子替换、持久写入和平台支持边界；原子 rename 不等于断电持久性证明。

采用Room作为唯一已提交安装事实源，保存稳定安装身份、来源身份、revision及完整组件清单，并保存最小Skill归属和会话选择。不可变文件内容在SQL事务外准备；所有生产可用性入口依据已提交引用。旧Connector JSON只作为一次性迁移输入，不继续双写；迁移完成标记与导入记录在同一事务发布。无法证明仅由包安装的旧Skill保留独立归属，不以迁移猜测删除用户资产。仅在无法由提交清单和引用推导恢复动作时增加最小恢复记录；不强制 PREPARING 等多阶段 journal、全局 generation 或通用 WAL 框架。

提交前进程死亡：旧版本仍有效，只清理可证明未提交且无引用的暂存内容。提交后进程死亡：按已提交事实恢复并重试清理；重复请求返回既有安装结果，不重放网络或工具调用。清理失败不得使完整的新版本不可用；确实缺失有效内容时仅标记受影响组件待修复。Secret 不写入清单或恢复记录。

更新不把未发送的旧配置调用静默改投新端点；调用前核对版本/目标及现有授权。已发送调用继续按原配置结算，相关引用释放后才清理；不声称更新能撤回远端副作用。不为此引入新的通用执行调度器。

### 5. 迁移、降级与范围

迁移保留旧记录及无法确定归属的用户资产，保持既有启用选择，不发起网络请求或运行 companion。迁移后缺失组件明确列为待修复，不伪造安装成功。

继续允许用户安装旧版本或重新安装，统一走同一安全替换路径。不建设历史版本浏览器、无限快照保留或一键回滚系统；不恢复被撤销凭据和旧审批。暂存及旧内容清理须有空间边界，且不能清理仍有归属或运行中引用的内容。

范围为 app 的 connector/marketplace/mcp 安装接线、会话能力面板和上下文/执行准入接线、extensions/skills 的必要归属及会话读取、测试及文档；若选用 core/storage 元数据，HXA-129 必须先补充具体 schema、迁移与恢复验证。公共工具格式和授权语义保持现有契约。

## Alternatives considered

- 先卸载再安装，加 catch 补偿：无法覆盖进程死亡，并会提前删除旧配置或凭据，不满足安全替换。
- 多阶段 journal、独立 owner/scope ledger 与全局 generation：预先锁定过多机制；仅保留实现正确提交和归属所需的最小状态。
- 每次更新要求审查代码 Diff：手机端操作成本高；采用简短更新摘要和实际变化的配置引导。
- 仅展示作者 Changelog：不能解决共享归属、失败保留及凭据绑定，不能替代上述底层保证。

## Consequences

用户获得失败时保留旧安装、未变配置免重设和共享内容不被误撤销的行为。每个会话能独立选择 Connector，选择与实际曝光、调用保持一致。维护成本集中在提交边界、引用归属及会话可用性读取；不增加多层 scope 继承、插件规划器、版本管理 UI 或重复审批。

## Verification

本次接受最小Room提交及会话启停契约；HXA-129开始实施，交付状态以任务与验收记录为准。现有 SkillRepository 有 session override 优先于 global override 的组件能力，不等于已经实现按 Connector 聚合的会话选择；接线时复用它而非重写其规则。已有成功重装及安装后重启用例不能证明安装途中强杀安全。

实施时先记录唯一提交点及所有可用性读取入口。测试覆盖候选部分准备失败、提交前取消/强杀、提交后清理前强杀、写入失败/低空间、提交响应丢失后重复请求、同名不同身份、两包共享、独立安装与包共存、未变凭据保留、目标变化重新配置和旧数据迁移。

会话验收覆盖 A 开/B 关、默认配置复制后互不影响、重启恢复、迁移、会话删除及可用的复制/fork 路径、共享 Skill 的剩余来源、工具单独禁用、更新保持选择但改变目标需配置，以及运行中关闭后的旧 schema/待审批调用不得执行。确认历史保留、后续请求不再注入、已发送调用正常结算，普通与 Goal Turn 行为一致。

确定性故障测试与 API29/36 × consumer/developer 的独占设备恢复测试互补；强杀必须发生在指定提交边界并观察新进程恢复，不以普通安装后重启替代。具体入口及交付证据归 HXA-129，文档检查通过不代表功能验收。

## Reconsider when

不能用单一提交事实覆盖现有 Registry 入口、必要恢复状态显著增长，或真实用户需要历史版本、每会话版本固定或多层 scope 继承时，单独审查具体需求和端侧成本，不提前扩建框架。

## References

- [实施任务 HXA-129](../../development/tasks/HXA-129.md)
- [内置市场契约](005-curated-marketplace.md)
- [签名索引契约](004-signed-index.md)
- [会话授权与工具禁用](../permissions/001-session-authorization.md)
- [主题入口](README.md)
