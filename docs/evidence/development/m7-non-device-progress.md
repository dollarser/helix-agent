...
| 079 | **完成**；JVM/App 单测与 API 29/36 production Task recovery fixture 各 2/2 | 独立 A2A Tool origin、固定 schema、Dispatcher/Policy/Approval/Audit 接线、Send/stream/Get/Cancel/Subscribe、持久 Task 对账、SSE `Last-Event-ID`、不明确送达 `NEEDS_REVIEW` 且不重发、不可信结果与 hash 复验/幂等复用的 Workspace Artifact 副本；详见 [完成记录](../../completion-records/HXA-079.md) |
...
当前专项验证命令仍以 [verification-matrix.md](../../development/verification-matrix.md) 的 M7 表为准。HXA-070～079 已逐项完成当前任务要求的 JVM/构建或 API 29/36 功能级设备链路，M7 在该范围内完成；M12 的物理真机、真实外部服务/网络、签名产物与完整 SBOM/notice 仍是明确发布门禁，不能由模拟器证据代替。
