...
依照 accepted ADR-PROVIDER-002/0026/0027，加入 developer 受管理 Copilot Provider、固定登录入口、Runtime Chat Completions 文本 adapter、短期 token 预刷新。复用现有 Chat encoder/decoder、共享 HTTP 限额与错误、v2 PFD 路由和 job journal。没有新增外部依赖或复制第三方代码；只增加既有内部 `provider:openai-chat` 模块依赖。consumer 不包含订阅，工具和图片仍关闭。
