...
所有者随后要求按实际功能更名。当前该包的生产路径负责订阅认证、凭据保管与模型请求转发，不提供通用 CLI 终端；用户可见名称改为 `Helix Subscription Provider`。内部 `com.helix.runtime.cli`、组件名、签名权限、Binder 描述符和凭据目录保留，历史 ADR 的 CLI Runtime 名称仍用于追溯。此为可回滚显示层调整，没有更改 ADR-PROVIDER-002/0007 的安全或生命周期边界。
