...
新增实际UI重试测试首先揭示失败Goal仍展示重试按钮的问题：断开模型流后Goal已进入FAILED，点击重试被现有Goal准入拒绝，没有第二个Turn。首轮 `hxa147-retry-ui-api29-consumer/` 失败日志与定向持久化诊断保留。ADR-GOAL-001与当前GoalReducer语义未改变；不通过放宽终态重启让界面测试通过。
