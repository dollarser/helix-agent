...
决策记录：[ADR-RUNTIME-001](../../adr/runtime/001-execution-domains.md)（accepted；用户授权，不代表实现验收）。
...
1. HXA-192：本地主机门禁已清零，继续补原计划尚缺的Plan用户闭环与授权隔离设备证据；主分支合并仍需独立授权。ADR-PERMISSIONS-002 已于 2026-09-14 接受有界架构，集成验收未完成；上方旧门禁数量不作为新执行结果。
